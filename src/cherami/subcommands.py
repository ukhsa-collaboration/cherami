...
def run(
    ...
    try:
        ...
        runner_cache: dict[str, PipelineRunner] = {}
        for sample_id in sample_ids:
            for pipeline_name in pipeline_names:
                ...
                try:
                    runner = runner_cache.get(pipeline_name)
                    if runner is None:
                        runner = PipelineRunner(
                            k8_api=k8_api,
                            sample_log=sample_log,
                        )
                        runner_cache[pipeline_name] = runner
...
def evaluate(
    ...
    for sample_id in sample_ids:
        for pipeline_name in pipeline_names:
            pipeline_config = parse_pipeline_config(pipeline_name, raw_config)
            pipeline = PIPELINES[pipeline_name](pipeline_config)
