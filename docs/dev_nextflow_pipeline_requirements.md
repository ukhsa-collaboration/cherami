...
```bash
nextflow run <path> -c <config.nf_config_path> -profile <config.nf_profiles> -r <config.version> [config.nf_extra_args] --server <config.server> --outdir <output_dir> --samplesheet <samplesheet>
```

## The following _MUST_ be implemented in the Nextflow pipeline:
...
Cherami always expects a pipeline to output directories/files created to a specific directory that is given with the `--outdir` flag.
...
### Use the 'version' in the pipeline config to fetch the branch/tag/commit

Once in production, the pipeline should have a release and tag with the version that Nextflow can pull. Cherami reads this tag (e.g. v1.0.0) from the pipeline config and adds it to the nextflow command (using `-r`). The version therefore needs adding to the pipeline config.
...
## **The following _MAY_ be implemented in the Nextflow pipeline:**

### Profiles

Profiles can be specified in the nextflow config associated with the pipeline and included in the repo. If profiles are specified in the pipeline's `nextflow.config`, add these to the cherami config for the pipeline in the pipeline config section.

Try to avoid using server-specific profiles.

### Server

The server given in the global section of the pipeline's cherami config is added to the nextflow command as the `server` parameter. If this is not needed, Nextflow will ignore the parameter (a warning might be given in the log; adding `param.server=null` into the nextflow.config can silence this.)

### Extra Nextflow Commandline Arguments

The Nextflow pipeline can take extra commandline arguments and be added to pipeline config such that cherami includes these in the command. Provide these in the pipeline config section as a list of strings. For example, you might wish to take a date and a specific input file, so the config should have `["--date 1970-01-01", "--input path/to/special_input.file"]`


## Writing Analyses to Onyx
If your pipeline creates any analysis results, these should be stored in Onyx in an analysis record. You _must_ use the onyx_analysis_helper to do this. The context for running the analysis _must_ also be captured - this includes the pipeline version, any database or dependency versions that if they were to change would cause different results. It is possible to pass the upstream context Cherami uses for the logic to determine whether the pipeline is run into the pipeline through the samplesheet.
